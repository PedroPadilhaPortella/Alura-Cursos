...
interface CartContextProps {
  ...
  actions: {
    ...
    removeFromCart: (book: CartBook) => void;
    updateItemQuantity: (book: CartBook, quantity: number) => void;
    resetCart: () => void;
  };
}
...
const cartReducer = (state: CartState, action: { type: string; payload?: any }): CartState => {
  switch (action.type) {
    ...
    case 'ADD_TO_CART':
      const bookAlreadyInTheCart = state.books.find((book) => book.id === action.payload.id);
      if (bookAlreadyInTheCart) {
...
const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const actions = {
    ...
    removeFromCart: (book: CartBook) => dispatch({ type: 'REMOVE_FROM_CART', payload: book }),
    updateItemQuantity: (book: CartBook, quantity: number) => dispatch({ type: 'UPDATE_ITEM_QUANTITY', payload: { ...book, quantity } }),
